/*
  ...
  1. New Tables
    - `email_preferences`
      - `id` (uuid, primary key)
      - `user_id` (uuid, references auth.users) - User who owns preferences
      - `email` (text) - Email address
      - `marketing_emails` (boolean) - Opt-in for marketing emails
      - `transactional_emails` (boolean) - Transactional emails (always true)
      - `deal_notifications` (boolean) - Daily deal alerts
      - `loyalty_notifications` (boolean) - Loyalty points updates
      - `order_updates` (boolean) - Order status updates
      - `newsletter_subscribed` (boolean) - Newsletter subscription
      - `brevo_contact_id` (text) - Brevo contact ID for syncing
      - `created_at` (timestamptz)
      - `updated_at` (timestamptz)

    - `email_logs`
      - `id` (uuid, primary key)
      - `user_id` (uuid, references auth.users, nullable) - Recipient user
      - `email` (text) - Recipient email address
      - `email_type` (text) - Type of email (order_confirmation, welcome, etc.)
      - `subject` (text) - Email subject
      - `template_id` (text) - Brevo template ID used
      - `status` (text) - Status: sent, failed, delivered, opened, clicked
    ...
    - `newsletter_subscribers`
      ...
      - `user_id` (uuid, references auth.users, nullable) - If registered user
      - `subscribed` (boolean) - Subscription status
      - `brevo_contact_id` (text) - Brevo contact ID
      - `source` (text) - Where they subscribed (footer, checkout, etc.)
  ...
  2. Security
    ...
    - Newsletter subscriptions managed by system

  3. Important Notes
    - Email preferences created automatically on user signup
    - All transactional emails respect user preferences
