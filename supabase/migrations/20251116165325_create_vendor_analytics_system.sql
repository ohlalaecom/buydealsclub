/*
  # Vendor Analytics & Market Intelligence System

  1. New Tables
    - `vendor_metrics`
      - Pre-calculated analytics per vendor
      - Total sales, revenue, conversion rates
      - Deal performance metrics
      - Updated daily for vendor dashboards
    
    - `vendor_search_insights`
      - Links search queries to vendor categories
      - Aggregates demand signals by category
      - Enables pricing recommendations
    
    - `market_demand`
      - Aggregated view of unmet customer requests
      - Grouped by category with price ranges
      - Powers vendor onboarding decisions
  
  2. Enhanced Unmet Requests
    - Add price_range, location, category_suggested fields
    - Auto-categorize into: hotels, spa_wellness, experiences, dining, retail
  
  3. Security
    - Vendors can only view their own metrics
    - Admin can view all vendor analytics
    - Public cannot access vendor data
  
  4. Performance
    - Materialized aggregates for fast dashboard loading
    - Indexes on vendor_id, category, date ranges
*/
