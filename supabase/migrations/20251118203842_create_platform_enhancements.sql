/*
  # Platform Enhancement Suite - Phase 6

  1. New Tables
    - `wishlists` - Customer favorite deals
    - `deal_alerts` - Price drop and stock alerts
    - `promo_codes` - Discount codes and coupons
    - `promo_code_usage` - Track code redemptions
    - `commission_transactions` - Platform revenue tracking
    - `vendor_payouts` - Payout history
    - `social_shares` - Track sharing activity
  
  2. Security
    - Enable RLS on all tables
    - Users manage their own wishlists/alerts
    - Only admins manage promo codes
    - Vendors see their own payouts
