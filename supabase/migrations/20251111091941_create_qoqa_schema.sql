/*
  # QoQa E-commerce Platform - Database Schema

  ## Overview
  This migration creates the complete database schema for a QoQa-inspired flash sale platform
  with community features, daily deals, and multi-category support.

  ## New Tables

  ### 1. `categories`
  - `id` (uuid, primary key) - Unique category identifier
  - `name` (text) - Category name (e.g., "Qsport", "Qwine", "Qooking")
  ...
  - `description` (text) - Full deal description
  ...
  - `deal_price` (numeric) - Discounted price
  - `discount_percentage` (integer) - Calculated discount percentage
  - `stock_quantity` (integer) - Available stock
  - `sold_quantity` (integer) - Number sold
  - `image_url` (text) - Main product image
  - `gallery_images` (jsonb) - Array of additional images
  - `start_time` (timestamptz) - Deal start time
  - `end_time` (timestamptz) - Deal end time (24h from start)
  ...
  - `deal_id` (uuid, foreign key) - Reference to deals table
  ...
  - `comment_id` (uuid, foreign key) - Reference to comments table
  - `user_id` (uuid, foreign key) - Reference to auth.users
  - `reaction_type` (text) - Type of reaction (like, love, laugh, wow)
  - `created_at` (timestamptz) - Creation timestamp
  ...
  - Frequently queried columns (category_id, user_id, deal_id)
  - Active deal lookups (is_active, start_time, end_time)
  - Username lookups
*/
